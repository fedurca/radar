    ...
    with DeviceFmcw() as device:
        ...
        try:
            while True:
                ...
                # --- CHANGE 2: Filter out readings below the threshold ---
                if peak_value >= PEAK_THRESHOLD:
                    distance_cm = distance_m * 100

                    # --- CHANGE 3: Reverse movement direction logic ---
                    if abs(speed_m_s) < metrics.speed_resolution_m_s:
                        direction = "Static"
                    elif speed_m_s > 0:
                        direction = "Receding" # Previously "Approaching"
                    else:
                        direction = "Approaching" # Previously "Receding"

                    # --- CHANGE 1: Add timestamp to the output ---
                    timestamp = datetime.now().strftime("%H:%M:%S.%f")[:-3] # Get timestamp with milliseconds
