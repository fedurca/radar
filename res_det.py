...
class DopplerAlgo:
    ...
    def _calculate_speed_axis(self, max_speed_m_s):
        num_doppler_bins = self.num_chirps_per_frame
        return np.linspace(-max_speed_m_s, max_speed_m_s, num_doppler_bins)
    ...
    def compute_doppler_map(self, frame_data, i_ant=0):
        ...
        # MTI filter to remove stationary DC offset
        ...
        try:
            ...
            return target_distance_m, target_speed_m_s, peak_value
        except IndexError:
            return 0.0, 0.0, 0.0
...
if __name__ == '__main__':
    ...
    # --- NEW: Master loop for handling reconnections ---
    while True:
        try:
            print("[INFO] Attempting to connect to radar device...")
            with DeviceFmcw() as device:
                print(f"[SUCCESS] Device connected. Radar SDK Version: {get_version_full()}")
                ...
                print(f"\nStarting measurements... (Threshold: {PEAK_THRESHOLD}) Press Ctrl+C to stop.")

                # --- MODIFIED: This is now the inner loop for processing data ---
                while True:
                    try:
                        ...
                        distance_m, speed_m_s, peak_value = algo.compute_doppler_map(antenna_samples)
                        ...
                        if peak_value >= PEAK_THRESHOLD:
                            ...
                            output = (f"{timestamp} | "
                                      f"Distance: {distance_cm:4.1f} cm | "
                        ...
                        # --- MODIFIED: Catch frame errors, assume disconnect, and break inner loop ---
                        print(f"\n[WARNING] Frame acquisition failed: {e}")
                        print("[INFO] Assuming device disconnected.")
                        break # Exit the inner loop to trigger a reconnect attempt

        except KeyboardInterrupt:
            print("\nProgram stopped by user.")
            sys.exit(0)
        except Exception as e:
            # This catches initial connection errors (e.g., ErrorNoDevice)
            print(f"[ERROR] Could not connect to device: {e}")

        # --- NEW: Wait before trying to reconnect ---
        print(f"[INFO] Retrying connection in 5 seconds...")
        time.sleep(5)