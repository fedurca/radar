...
<head>
    ...
    <style>
        ...
        body.dark-mode {
            ...
            --color-approaching: #28a745; --color-receding: #ff4d4f; --color-static: #4dabf7; --color-none: transparent;
        }
        * { box-sizing: border-box; }
        ...
        .data-grid { display: grid; grid-template-columns: repeat(3, 1fr); gap: 1rem; margin-top: 1rem; }
        ...
        .sector-grid { display: grid; grid-template-columns: repeat(16, 1fr); gap: 2px; width: 100%; height: 40px; margin-top: 1.5rem; border: 1px solid var(--border-color); border-radius: 6px; padding: 2px; }
        .sector { background-color: var(--secondary-bg-color); border-radius: 2px; transition: background-color 0.2s; }
...
<body>
    ...
    <script>
        (() => {
            ...
            for(let i=0; i<16; i++) { const sector = document.createElement('div'); sector.className = 'sector'; ui.sectorGrid.appendChild(sector); }
            ...
            function updateUI(data) {
                ...
                if (displayData) {
                    ...
                    ui.min_dist.textContent = (displayData.min_dist_cm || 0).toFixed(1); ui.max_dist.textContent = (displayData.max_dist_cm || 0).toFixed(1);
                    ui.min_speed.textContent = (displayData.min_speed_ms || 0).toFixed(2); ui.max_speed.textContent = (displayData.max_speed_ms || 0).toFixed(2);
                    ui.min_peak.textContent = (displayData.min_peak || 0).toFixed(4); ui.max_peak.textContent = (displayData.max_peak || 0).toFixed(4);
        ...
        try:
            with state_lock:
                ...
                if reconfigure_needed:
                    ...
                    stats = reset_stats(); sector_states = ['none'] * 16
            ...
            if device is None:
                ...
                # --- FIX pro vysoké frekvence ---
                chirp_loop_temp = sequence.loop.sub_sequence.contents
                device.sequence_from_metrics(metrics, chirp_loop_temp)
                chirp_duration_s = 0.000128 # Typická hodnota pro 64 vzorků @ 1MHz, bezpečný odhad
                required_time_s = chirp_duration_s * num_chirps
                available_time_s = 1.0 / frate
                
                if required_time_s > available_time_s * 0.9: # 10% buffer
                    original_chirps = num_chirps
                    num_chirps = int((available_time_s * 0.9) / chirp_duration_s)
                    log_and_broadcast("warning", f"Konfigurace nekompatibilní. Počet chirpů automaticky snížen z {original_chirps} na {num_chirps}.", loop)
            ...
            if peak_value >= current_peak_threshold:
                ...
                with state_lock:
                    ...
                    sector_index = int((smoothed_distance / max_range_cm_current) * 16)
                    css_direction = 'static'
                    if direction == "Přibližování": css_direction = "approaching"
                    elif direction == "Vzdalování": css_direction = "receding"
                    if 0 <= sector_index < 16: sector_states[sector_index] = css_direction
            else:
            ...
            smoothed_distance, smoothed_speed, device, connection_start_time = None, None, None, None
            stats = reset_stats()
            with state_lock: sector_states = ['none'] * 16
            time.sleep(3)
