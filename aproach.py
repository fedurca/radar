...
class DopplerAlgo:
    ...
    def compute_doppler_map(self, frame_data, i_ant=0):
        ...
        try:
            ...
            # This block is for safety and debugging, should not be hit often
            return 0.0, 0.0, 0.0
    ...
    with DeviceFmcw() as device:
        ...
        try:
            while True:
                ...
                # --- CHANGE 1: Filter out speed when no target is detected ---
                if distance_cm < 1: # If distance is less than 1 cm
                    ...
                    # --- CHANGE 2 & 3: Format distance and speed as whole numbers ---
                    output = (f"Distance: {distance_cm:.0f} cm | "
                              f"Speed: {speed_m_s:+.0f} m/s "
                              f"({direction}) | "
