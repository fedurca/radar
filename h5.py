...
<body>
    ...
    <script>
        (() => {
            ...
            let maxDistanceCm = 800, maxSpeedMs = 3, maxPeak = 10, langDict = {}, lastValidData = null, isPlotPaused = false;
            ...
            function drawPlot(history) {
                ...
                // Y-Axis
                plotCtx.beginPath(); plotCtx.moveTo(30, 0); plotCtx.lineTo(30, h-20); plotCtx.stroke();
                plotCtx.textAlign = "right"; plotCtx.textBaseline = "middle";
                [0, 0.25, 0.5, 0.75, 1].forEach(p => {
                    const y = h - 20 - (p * (h-20));
                    plotCtx.fillText((p * maxDistanceCm).toFixed(0), 25, y);
                    plotCtx.beginPath(); plotCtx.moveTo(25, y); plotCtx.lineTo(30, y); plotCtx.stroke();
                });

                // X-Axis
                const frate = parseInt(ui.frateSelector.value, 10) || 20;
                const timeSpan = history.length / frate;
                plotCtx.beginPath(); plotCtx.moveTo(30, h-20); plotCtx.lineTo(w, h-20); plotCtx.stroke();
                plotCtx.textAlign = "center"; plotCtx.textBaseline = "top";
                 [0, 0.25, 0.5, 0.75, 1].forEach(p => {
                    const x = 30 + (p * (w-30));
                    plotCtx.fillText(`-${(timeSpan * (1-p)).toFixed(1)}s`, x, h-15);
                    plotCtx.beginPath(); plotCtx.moveTo(x, h-20); plotCtx.lineTo(x, h-15); plotCtx.stroke();
                });
                ...
                for (let i = 1; i < history.length; i++) {
                    ...
                    const x1 = 30 + (i-1)/(history.length-1)*(w-30), x2 = 30 + i/(history.length-1)*(w-30);
    ...
    while True:
        try:
            ...
            if device is None:
                ...
                # --- FIX: Kontrola a úprava parametrů pro vysoké frekvence ---
                ...
                chirp_duration_s = chirp_loop.loop.sub_sequence.contents.chirp.duration_s
                
                required_time_s = chirp_duration_s * num_chirps
                available_time_s = 1.0 / frate
                
                if required_time_s > available_time_s * 0.95: # 5% buffer
